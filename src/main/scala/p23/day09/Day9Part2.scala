...
object Day9Part2 {
  ...
  private def getHistoryValue(lines: List[List[Long]]): Long = {
    lines.map(_.head).reverse.foldLeft(0L)((acc, e) => e - acc)
  }
  ...
  def check(all: String): Long =
    parseLines(all).map(l => getHistoryValue(allLines(l))).sum
}
